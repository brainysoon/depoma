...
def extract_data(file_path, save_path, limit=20, x_limit=3, y_limit=6):
    """执行程序
    Args:
        limit: 只输出句子长度小于limit的句子
    """
    from flaskr.ml.word_sequence import WordSequence
    ...
    print('dump')
    chatbot_pkl = save_path + '.chatbot.pkl'
    ws_pkl = save_path + '.ws.pkl'

    pickle.dump(
        (x_data, y_data),
        open(chatbot_pkl, 'wb')
    )
    pickle.dump(ws_input, open(ws_pkl, 'wb'))

    print('done')

    return chatbot_pkl, ws_pkl
